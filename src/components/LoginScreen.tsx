...
export default function LoginScreen({ 
  ...
  const [deviceBoundUser, setDeviceBoundUser] = useState<User | null>(null);
  ...
  // 1 Phone = 1 Account: Check if this physical client device is bound to an active member account
  useEffect(() => {
    let isMounted = true;
    async function checkDeviceBinding() {
      try {
        const clientDevId = getClientDeviceId();
        const boundResult = await getDeviceBoundUser(clientDevId);
        if (isMounted && boundResult && boundResult.user) {
          const u = boundResult.user;
          setDeviceBoundUser(u);
          setFoundUser(u);
          setPhoneNumber(u.phone || '');
          setIsRegistering(false);
          // If approved, directly prompt for PIN
          if (u.approved !== false) {
            if (!u.pin || u.pin.trim() === '' || u.isPendingPin === true || u.pinSet === false) {
        ...
        console.warn("Device binding check err:", err);
      }
    }
    checkDeviceBinding();
  ...
  const handleCompleteLogin = async (liveUser: User, finalPin: string, finalPassword?: string) => {
    ...
    // STRICT 1 PHONE = 1 ACCOUNT: Ensure this phone is not registered to a different member
    const boundToDev = await getDeviceBoundUser(clientDevId);
    if (boundToDev && boundToDev.user.uid !== liveUser.uid && liveUser.role !== 'admin') {
      setError(`⚠️ এই ফোনটি ইতিমধ্যে ${boundToDev.user.name || 'অন্য সদস্য'} (আইডি: ${boundToDev.user.memberId || 'N/A'})-এর অ্যাকাউন্টের সাথে নিবন্ধিত। নীতি অনুযায়ী একটি ফোনে কেবলমাত্র একটি অ্যাকাউন্ট পরিচালনা করা যাবে। অন্যের অ্যাকাউন্টে প্রবেশ সম্পূর্ণ নিষিদ্ধ।`);
      setLoading(false);
      return;
    }
  ...
  const handleNextStep = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (foundResult) {
        ...
        // STRICT 1 PHONE = 1 ACCOUNT CHECK 1: Is this phone already bound to another member?
        const clientDevId = getClientDeviceId();
        const clientFp = getDeviceFingerprint();

        if (deviceBoundUser && deviceBoundUser.uid !== userData.uid && userData.role !== 'admin') {
          setError(`⚠️ এই ফোনটি ইতিমধ্যে ${deviceBoundUser.name || 'একজন সদস্য'} (আইডি: ${deviceBoundUser.memberId || 'N/A'})-এর অ্যাকাউন্টে সংরক্ষিত ও সংযুক্ত। নীতি অনুযায়ী একটি ফোনে কেবলমাত্র একটি অ্যাকাউন্ট পরিচালনা করা যাবে। অন্যের অ্যাকাউন্টে প্রবেশ সম্পূর্ণ নিষিদ্ধ।`);
          setLoading(false);
          return;
        }

        // STRICT 1 PHONE = 1 ACCOUNT CHECK 2: Is this user's account already locked to another phone?
        if (userData.currentDeviceId && userData.role !== 'admin' && !userData.deviceLockBypassed) {
          const isAuth = isSameDevice(userData.currentDeviceId, userData.deviceFingerprint, clientDevId, clientFp, userData.activeDeviceTokens);
          if (!isAuth) {
  ...
  const handleSetInitialPin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure this phone is not bound to a different member
      const boundCheck = await getDeviceBoundUser(clientDevId);
      if (boundCheck && boundCheck.user.uid !== foundUser.uid && foundUser.role !== 'admin') {
        setError(`⚠️ এই ফোনটি ইতিমধ্যে ${boundCheck.user.name || 'অন্য সদস্য'} (আইডি: ${boundCheck.user.memberId || 'N/A'})-এর অ্যাকাউন্টের সাথে নিবন্ধিত। নীতি অনুযায়ী একটি ফোনে কেবলমাত্র একটি অ্যাকাউন্ট পরিচালনা করা যাবে। অন্যের অ্যাকাউন্টে প্রবেশ সম্পূর্ণ নিষিদ্ধ।`);
        setLoading(false);
        return;
      }
  ...
  const handleRegisterWithPin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // STRICT 1 PHONE = 1 ACCOUNT: Prevent multiple accounts on the same phone
      const deviceBound = await getDeviceBoundUser(clientDevId);
      if (deviceBound && deviceBound.user) {
        setError(`⚠️ এই ফোনে ইতিমধ্যে একটি অ্যাকাউন্ট (${deviceBound.user.name}, আইডি: ${deviceBound.user.memberId}) নিবন্ধিত রয়েছে। নীতি অনুযায়ী একটি ফোনে কেবলমাত্র একটি অ্যাকাউন্ট পরিচালনা করা যাবে। একাধিক অ্যাকাউন্ট খোলা সম্পূর্ণ নিষিদ্ধ।`);
        setLoading(false);
        return;
      }
      ...
      if (!midCheck.empty) {
        generatedMemberId = await getNextSequentialMemberId();
      }
      ...
      if (existingDocSnap.exists()) {
        setError('নিবন্ধন সতর্কতা: এই অ্যাকাউন্টের ডাটা সার্ভারে ইতিমধ্যে বিদ্যমান রয়েছে। নিরাপত্তা স্বার্থে কোনো ডাটা মুছে ফেলা বা ওভাররাইট করা সম্পূর্ণ নিষিদ্ধ। অনুগ্রহ করে সরাসরি লগইন করুন।');
        setLoading(false);
        return;
      }
      ...
      // Set user record in Firestore under permanent unique doc ID WITHOUT overwrite
      await setDoc(userDocRef, newUser, { merge: false });
              ...
              >
                ...
                <div className="flex bg-slate-100/90 p-1.5 rounded-2xl border border-slate-200/80">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (deviceBoundUser) {
                        setError(`⚠️ এই ফোনে ইতিমধ্যে ${deviceBoundUser.name} (আইডি: ${deviceBoundUser.memberId})-এর অ্যাকাউন্ট সংযুক্ত রয়েছে। নীতি অনুযায়ী একটি ফোনে একটি অ্যাকাউন্ট ব্যবহার করা যাবে। নতুন অ্যাকাউন্ট খুলতে হলে অন্য ফোন ব্যবহার করুন।`);
                        return;
                      }
                ...
                {deviceBoundUser && (
                  <div className="bg-emerald-50 border border-emerald-300 p-3 rounded-2xl flex items-start gap-2.5 text-xs text-emerald-950 font-bold shadow-2xs">
                    <ShieldCheck className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="font-extrabold text-emerald-900">এই ফোনে সংরক্ষিত সদস্য:</span>
                        <span className="bg-emerald-200/90 text-emerald-900 px-2 py-0.5 rounded-lg text-[11px] font-black">{deviceBoundUser.name}</span>
                      </div>
                      <div className="text-[11px] text-emerald-800 font-mono">
                        আইডি: <span className="font-bold">{deviceBoundUser.memberId}</span> • মোবাইল: <span className="font-bold">{deviceBoundUser.phone}</span>
                      </div>
                      <div className="text-[10px] text-slate-500 font-medium">
                        🛡️ নিরাপত্তা নীতি: ১ ফোনে ১ অ্যাকাউন্ট। এই ফোনে শুধুমাত্র এই সদস্যই প্রবেশ করতে পারবেন।
                      </div>
                    </div>
                  </div>
                )}
