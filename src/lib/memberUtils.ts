...
/**
 * Checks if the physical client device is already bound to an active member account.
 * Enforces strict 1-Phone = 1-Account policy:
 * Returns the bound User if found, or null if this device is unbound / released.
 */
export async function getDeviceBoundUser(clientDevId?: string): Promise<{ docId: string; user: User } | null> {
  const targetDevId = (clientDevId || getClientDeviceId()).trim();
  if (!targetDevId) return null;

  try {
    const q = query(
      collection(db, 'users'),
      where('currentDeviceId', '==', targetDevId)
    );
    const snap = await getDocs(q);
    if (!snap.empty) {
      for (const d of snap.docs) {
        const u = d.data() as User;
        // Ignore deleted users or admins or users with explicit device bypass
        if ((u as any).deleted === true || (u as any).isDeleted === true) continue;
        if (u.role === 'admin' || u.role === 'sub_admin' || u.uid === 'admin_master') continue;
        if (u.deviceLockBypassed === true) continue;

        return { docId: d.id, user: { ...u, uid: d.id } };
      }
    }
  } catch (err) {
    console.warn("getDeviceBoundUser check warning:", err);
  }
  return null;
}
